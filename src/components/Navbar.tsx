...
type MenuKey = "think" | "pathways" | "boarding" | "talent" | "joinUs" | "fees" | "getAccess";

const MENU: Record<MenuKey, MenuGroup> = {
  think: { label: "Think Pioneer", to: "/think", items: [{ to: "/different", label: "Think Different" }, { to: "/leadership", label: "Our Imaginative Team" }] },
  pathways: { label: "Think Pathways", to: "/pathways", items: [{ to: "/detailed-pathways", label: "Detailed Pathways" }, { to: "/cadet", label: "Think Aviation" }, { to: "/mandarin", label: "Think Mandarin" }, { to: "/seafarers", label: "Think Maritime" }] },
  boarding: { label: "Think Boarding", to: "/boarding-life", items: [{ to: "/farm-2-fork", label: "Farm to Fork" }, { to: "/laundry", label: "Laundry" }, { to: "/chaplaincy", label: "Chaplaincy" }, { to: "/nursing-care", label: "Nursing Care" }, { to: "/safety", label: "Safety & Security" }] },
  talent: { label: "Think Talent", to: "/talent", items: [{ to: "/band", label: "Band Life" }, { to: "/football", label: "Soccer" }, { to: "/swimmers", label: "Swimming" }, { to: "/cycling", label: "Cycling" }, { to: "/skating", label: "Skating" }, { to: "/golf", label: "Golf" }, { to: "/farmers", label: "Farmers Club" }, { to: "/chess", label: "Chess Club" }, { to: "/coding", label: "Robotics Club" }, { to: "/basket", label: "Basketball" }] },
  joinUs: { label: "Join Us", to: "/join", items: [{ to: "/grade7", label: "Join Grade 7" }, { to: "/grade8", label: "Join Grade 8" }, { to: "/grade9", label: "Join Grade 9" }, { to: "https://enquireto.pioneergroupofschools.co.ke/main/register?school=pioneerjunioracademy.co.ke/SignIn?ReturnUrl=%2F", label: "Enquire" }] },
  fees: { label: "Fees", to: "/fees", items: [{ to: "/grade7fee", label: "Grade 7 Fees" }, { to: "/grade8fee", label: "Grade 8 Fees" }, { to: "/grade9fee", label: "Grade 9 Fees" }, { to: "/terms", label: "Terms & Conditions" }] },
...
const ORDER: MenuKey[] = ["think", "pathways", "boarding", "talent", "joinUs", "fees"];

const isExternal = (href: string) => href.startsWith("http://") || href.startsWith("https://");

const Divider = () => <span className="hidden lg:block mx-1 h-5 w-px bg-white/40" />;

const LinkItem = ({ item, onClick }: { item: MenuItem; onClick?: () => void }) =>
  isExternal(item.to) ? (
    <a href={item.to} target="_blank" rel="noopener noreferrer" onClick={onClick} className="block px-3 py-2 rounded-md bg-white text-black hover:bg-black hover:text-white">
      {item.label}
    </a>
  ) : (
    <NavLink to={item.to} onClick={onClick} className="block px-3 py-2 rounded-md bg-white text-black hover:bg-black hover:text-white">
      {item.label}
    </NavLink>
  );
    ...
    <header className="sticky top-0 z-50 bg-[#0f1b2b] text-white">
      <div ref={navRef} className="max-w-8xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center text-base font-bold">
          ...
          {ORDER.map((k, i) => (
            <React.Fragment key={k}>
              <div className="relative" onMouseEnter={() => openMenu(k)} onMouseLeave={scheduleCloseMenu}>
          ...
          <div className="relative" onMouseEnter={() => openMenu("getAccess")} onMouseLeave={scheduleCloseMenu}>
        ...
        <button
          ...
          className="lg:hidden inline-flex items-center justify-center rounded-md p-2 bg-[#74d1f6] text-white hover:bg-[#b9782a] transition"
        >
